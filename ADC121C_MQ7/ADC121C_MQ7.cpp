...
/**************************************************************************/
/*
        Instantiates a new ADC121C_MQ7 class with appropriate properties
*/
/**************************************************************************/
void ADC121C_MQ7::getAddr_ADC121C_MQ7(uint8_t i2cAddress)
{
    ADC121C_MQ7_i2cAddress = i2cAddress;
    ADC121C_MQ7_conversionDelay = ADC121C_MQ7_CONVERSIONDELAY;
}
...
*/
/**************************************************************************/
bool ADC121C_MQ7::getAlertStatus()
{
    // high order bit is the alert flag, mask off the rest
    bool alert = (readRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_CONVERSION) & 0x8000) ? true : false;
    
    if (alert)
    {
        // Read the alert low and high values
        // And set the appropriate member variables
        uint8_t status = readRegister8(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_ALERT_STATUS);
        // Under Range Alert Flag
        if (status & 0x01)
            ADC121C_MQ7_alertLow = true;
        else
            ADC121C_MQ7_alertLow = false;
        
        // Over Range Alert Flag
        if (status & 0x02)
            ADC121C_MQ7_alertHigh = true;
        else
            ADC121C_MQ7_alertHigh = false;
    }
...
*/
/**************************************************************************/
bool ADC121C_MQ7::alertLowTriggered()
{
    return ADC121C_MQ7_alertLow;
}
...
*/
/**************************************************************************/
bool ADC121C_MQ7::alertHighTriggered()
{
    return ADC121C_MQ7_alertHigh;
}
...
*/
/**************************************************************************/
void ADC121C_MQ7::clearAlertStatus()
{
    // Zero out both the low and high alert flags
    uint8_t status = 0x03;
    writeRegister8(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_ALERT_STATUS, status);
...
*/
/**************************************************************************/
void ADC121C_MQ7::setCycleTime(ADC121C_MQ7CycleTime_t cycletime)
{
    ADC121C_MQ7_cycletime = cycletime;
}
...
*/
/**************************************************************************/
ADC121C_MQ7CycleTime_t ADC121C_MQ7::getCycleTime()
{
    return ADC121C_MQ7_cycletime;
}
...
*/
/**************************************************************************/
void ADC121C_MQ7::setAlertHold(ADC121C_MQ7AlertHold_t alerthold)
{
    ADC121C_MQ7_alerthold = alerthold;
}
...
*/
/**************************************************************************/
ADC121C_MQ7AlertHold_t ADC121C_MQ7::getAlertHold()
{
    return ADC121C_MQ7_alerthold;
}
...
*/
/**************************************************************************/
void ADC121C_MQ7::setAlertFlag(ADC121C_MQ7AlertFlag_t alertflag)
{
    ADC121C_MQ7_alertflag = alertflag;
}
...
*/
/**************************************************************************/
ADC121C_MQ7AlertFlag_t ADC121C_MQ7::getAlertFlag()
{
    return ADC121C_MQ7_alertflag;
}
...
*/
/**************************************************************************/
void ADC121C_MQ7::setAlertPin(ADC121C_MQ7AlertPin_t alertpin)
{
    ADC121C_MQ7_alertpin = alertpin;
}
...
*/
/**************************************************************************/
ADC121C_MQ7AlertPin_t ADC121C_MQ7::getAlertPin()
{
    return ADC121C_MQ7_alertpin;
}
...
*/
/**************************************************************************/
void ADC121C_MQ7::setPolarity(ADC121C_MQ7Polarity_t polarity)
{
    ADC121C_MQ7_polarity = polarity;
}
...
*/
/**************************************************************************/
ADC121C_MQ7Polarity_t ADC121C_MQ7::getPolarity()
{
    return ADC121C_MQ7_polarity;
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::setAlertLowLimit(uint16_t limit)
{
    // mask off the invalid bits in case they were set
    limit &= 0x0FFF;
    return writeRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_LOW_LIMIT, limit);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::setAlertHighLimit(uint16_t limit)
{
    // mask off the invalid bits in case they were set
    limit &= 0x0FFF;
    return writeRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_HIGH_LIMIT, limit);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::setHysteresis(uint16_t hysteresis)
{
    // mask off the invalid bits in case they were set
    hysteresis &= 0x0FFF;
    return writeRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_HYSTERESIS, hysteresis);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::getLowestConversion()
{
    return readRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_LOWCONV);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::getHighestConversion()
{
    return readRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_HIGHCONV);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::clearLowestConversion()
{
    uint16_t lowClear = 0x0FFF;
    return writeRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_LOWCONV, lowClear);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::clearHighestConversion()
{
    uint16_t highClear = 0x0000;
    return writeRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_HIGHCONV, highClear);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ7::Measure_Voltage()
{
    // Start with default values
    // Set the Cycle Time
    uint8_t config = ADC121C_MQ7_cycletime;
    
    // Set the Alert Hold Status
    config |= ADC121C_MQ7_alerthold;
    
    // Set the Alert Flag Enable Status
    config |= ADC121C_MQ7_alertflag;
    
    // Set the Alert Pin Enable Status
    config |= ADC121C_MQ7_alertpin;
    
    // Set the Polarity
    config |= ADC121C_MQ7_polarity;

    // Write config register to the ADC
    writeRegister8(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_CONFIG, config);

    // Wait for the conversion to complete
    delay(ADC121C_MQ7_conversionDelay);

    // Read the conversion result
    // 12-bit unsigned result for the ADC121C_MQ7
    return readRegister(ADC121C_MQ7_i2cAddress, ADC121C_MQ7_REG_POINTER_CONVERSION);
}

/**************************************************************************/
/*
        This function returns the sensor resistance in clear air (Ro).
        Ro: Sensor Resistance at 1000ppm of CO in the Clean Air
*/
/**************************************************************************/
void ADC121C_MQ7::Calibration(float Ro)
{
    _Ro = Ro;
...
/*
        ...
        Measure_Resistance is to caculate the sensor resistance in clear air (Ro)
        This assumes that the sensor is in the clean air.
        Ro: Sensor Resistance at 1000ppm of CO in the Clean Air
*/
/**************************************************************************/
void ADC121C_MQ7::Calibration()
{
    float Ro = 0;
    for (int i = 0; i < ADC121C_MQ7_SAMPLE_TIMES; i++)
    {
        Ro += Measure_Resistance(Measure_Voltage());
        delay(ADC121C_MQ7_SAMPLE_INTERVAL);
    }
    Ro = Ro/ADC121C_MQ7_SAMPLE_TIMES;
...
*/
/**************************************************************************/
float ADC121C_MQ7::Measure_Resistance(int rawadc)
{
...
float ADC121C_MQ7::Measure_Rs()
{
    ...
    Rs = Rs / ADC121C_MQ7_SAMPLE_TIMES;
    return Rs;
}
...
*/
/**************************************************************************/
float ADC121C_MQ7::Measure_Scaled(float a, float b)
{
...
*/
/**************************************************************************/
float ADC121C_MQ7::Measure_RatioRsRo()
{
...
/*
        ...
        we get Carbon Monoxide gas ppm value as output.
 */
/**************************************************************************/
float ADC121C_MQ7::Measure_CarbonMonoxide(float a, float b)
{
