...
void setup(void) 
{
    ...
    // sts.setTempMode(TEMP_HOLD);        // Measure Temperature, Hold Master Mode
...
void loop(void)
{
    ...
    if (error == 0)
    {
        ...
        Serial.println("Getting Temperature Readings from STS21");
        Serial.println(" ");
        // Read and print out the temperature, then convert to C and F scales
        temp = sts.Measure_Temp();
    ...
    {
        Serial.println("STS21 Disconnected!");
        Serial.println(" ");
        Serial.println("        ************        ");
        Serial.println(" ");
    }
