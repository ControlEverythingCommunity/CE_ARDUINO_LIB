...
/**************************************************************************/
/*
        Instantiates a new ADC121C_MQ3 class with appropriate properties
*/
/**************************************************************************/
void ADC121C_MQ3::getAddr_ADC121C_MQ3(uint8_t i2cAddress)
{
    ADC121C_MQ3_i2cAddress = i2cAddress;
    ADC121C_MQ3_conversionDelay = ADC121C_MQ3_CONVERSIONDELAY;
}
...
*/
/**************************************************************************/
bool ADC121C_MQ3::getAlertStatus()
{
    // high order bit is the alert flag, mask off the rest
    bool alert = (readRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_CONVERSION) & 0x8000) ? true : false;
    
    if (alert)
    {
        // Read the alert low and high values
        // And set the appropriate member variables
        uint8_t status = readRegister8(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_ALERT_STATUS);
        // Under Range Alert Flag
        if (status & 0x01)
            ADC121C_MQ3_alertLow = true;
        else
            ADC121C_MQ3_alertLow = false;
        
        // Over Range Alert Flag
        if (status & 0x02)
            ADC121C_MQ3_alertHigh = true;
        else
            ADC121C_MQ3_alertHigh = false;
    }
...
*/
/**************************************************************************/
bool ADC121C_MQ3::alertLowTriggered()
{
    return ADC121C_MQ3_alertLow;
}
...
*/
/**************************************************************************/
bool ADC121C_MQ3::alertHighTriggered()
{
    return ADC121C_MQ3_alertHigh;
}
...
*/
/**************************************************************************/
void ADC121C_MQ3::clearAlertStatus()
{
    // Zero out both the low and high alert flags
    uint8_t status = 0x03;
    writeRegister8(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_ALERT_STATUS, status);
...
*/
/**************************************************************************/
void ADC121C_MQ3::setCycleTime(ADC121C_MQ3CycleTime_t cycletime)
{
    ADC121C_MQ3_cycletime = cycletime;
}
...
*/
/**************************************************************************/
ADC121C_MQ3CycleTime_t ADC121C_MQ3::getCycleTime()
{
    return ADC121C_MQ3_cycletime;
}
...
*/
/**************************************************************************/
void ADC121C_MQ3::setAlertHold(ADC121C_MQ3AlertHold_t alerthold)
{
    ADC121C_MQ3_alerthold = alerthold;
}
...
*/
/**************************************************************************/
ADC121C_MQ3AlertHold_t ADC121C_MQ3::getAlertHold()
{
    return ADC121C_MQ3_alerthold;
}
...
*/
/**************************************************************************/
void ADC121C_MQ3::setAlertFlag(ADC121C_MQ3AlertFlag_t alertflag)
{
    ADC121C_MQ3_alertflag = alertflag;
}
...
*/
/**************************************************************************/
ADC121C_MQ3AlertFlag_t ADC121C_MQ3::getAlertFlag()
{
    return ADC121C_MQ3_alertflag;
}
...
*/
/**************************************************************************/
void ADC121C_MQ3::setAlertPin(ADC121C_MQ3AlertPin_t alertpin)
{
    ADC121C_MQ3_alertpin = alertpin;
}
...
*/
/**************************************************************************/
ADC121C_MQ3AlertPin_t ADC121C_MQ3::getAlertPin()
{
    return ADC121C_MQ3_alertpin;
}
...
*/
/**************************************************************************/
void ADC121C_MQ3::setPolarity(ADC121C_MQ3Polarity_t polarity)
{
    ADC121C_MQ3_polarity = polarity;
}
...
*/
/**************************************************************************/
ADC121C_MQ3Polarity_t ADC121C_MQ3::getPolarity()
{
    return ADC121C_MQ3_polarity;
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::setAlertLowLimit(uint16_t limit)
{
    // mask off the invalid bits in case they were set
    limit &= 0x0FFF;
    return writeRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_LOW_LIMIT, limit);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::setAlertHighLimit(uint16_t limit)
{
    // mask off the invalid bits in case they were set
    limit &= 0x0FFF;
    return writeRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_HIGH_LIMIT, limit);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::setHysteresis(uint16_t hysteresis)
{
    // mask off the invalid bits in case they were set
    hysteresis &= 0x0FFF;
    return writeRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_HYSTERESIS, hysteresis);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::getLowestConversion()
{
    return readRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_LOWCONV);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::getHighestConversion()
{
    return readRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_HIGHCONV);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::clearLowestConversion()
{
    uint16_t lowClear = 0x0FFF;
    return writeRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_LOWCONV, lowClear);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::clearHighestConversion()
{
    uint16_t highClear = 0x0000;
    return writeRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_HIGHCONV, highClear);
}
...
*/
/**************************************************************************/
uint16_t ADC121C_MQ3::Measure_Voltage()
{
    // Start with default values
    // Set the Cycle Time
    uint8_t config = ADC121C_MQ3_cycletime;
    
    // Set the Alert Hold Status
    config |= ADC121C_MQ3_alerthold;
    
    // Set the Alert Flag Enable Status
    config |= ADC121C_MQ3_alertflag;
    
    // Set the Alert Pin Enable Status
    config |= ADC121C_MQ3_alertpin;
    
    // Set the Polarity
    config |= ADC121C_MQ3_polarity;

    // Write config register to the ADC
    writeRegister8(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_CONFIG, config);

    // Wait for the conversion to complete
    delay(ADC121C_MQ3_conversionDelay);

    // Read the conversion result
    // 12-bit unsigned result for the ADC121C_MQ3
    return readRegister(ADC121C_MQ3_i2cAddress, ADC121C_MQ3_REG_POINTER_CONVERSION);
}

/**************************************************************************/
/*
        This function returns the sensor resistance in clear air (Ro).
        Ro means Resistance of sensor in 0. 4mg/l Alcohol
*/
/**************************************************************************/
void ADC121C_MQ3::Calibration(float Ro)
{
    _Ro = Ro;
...
*/
/**************************************************************************/
void ADC121C_MQ3::Calibration()
{
    float Ro = 0;
    for (int i = 0; i < ADC121C_MQ3_SAMPLE_TIMES; i++)
    {
        Ro += Measure_Resistance(Measure_Voltage());
        delay(ADC121C_MQ3_SAMPLE_INTERVAL);
    }
    Ro = Ro/ADC121C_MQ3_SAMPLE_TIMES;
...
*/
/**************************************************************************/
float ADC121C_MQ3::Measure_Resistance(int rawadc)
{
...
/*
        ...
        Rs means Resistance Different Gases
*/
/**************************************************************************/
float ADC121C_MQ3::Measure_Rs()
{
    float Rs = 0;
    for (int i = 0; i < ADC121C_MQ3_SAMPLE_TIMES; i++)
    {
    ...
    Rs = Rs / ADC121C_MQ3_SAMPLE_TIMES;
    return Rs;
}
...
*/
/**************************************************************************/
float ADC121C_MQ3::Measure_Scaled(float a, float b)
{
...
*/
/**************************************************************************/
float ADC121C_MQ3::Measure_RatioRsRo()
{
...
/*
        ...
        we get Alcohol MgL value as output.
 */
/**************************************************************************/
float ADC121C_MQ3::Measure_AlcoholMgL(float a, float b)
{
...
/*
        By using the slope and a point of the line (logarithmic value of ppm),
        we get Alcohol ppm value as output.
 */
/**************************************************************************/
float ADC121C_MQ3::Measure_Alcoholppm(float a, float b)
{
    return (float)Measure_Scaled(a, b);
}
