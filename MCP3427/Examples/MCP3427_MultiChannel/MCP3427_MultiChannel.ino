...
MCP3427 MCP(0); // Declaration of MCP3427: A2, A1, A0 bits (000, 0x68)

void setup()
{
    // Start serial communication and set baud rate = 9600
    Serial.begin(9600);
    Serial.println("MCP3427 Analog to Digital Converter");
    Serial.println("Getting ADC Readings from Channel 1 and 2");
    Serial.println("        ************************************            ");
    Serial.println("        ");
    delay(500);
}
...
void loop()
{
    ...
    if (error == 0)
    {
        ...
        for(int i=1;i<=2;i++)
        {
            // MCP3427 is configured to channel i with 14 bits resolution, continuous mode and gain defined to 1
            // This arrangement of the mentioned paarmeters can be changed as per convenience
            MCP.SetConfiguration(i,14,1,1);
    ...
    {
        Serial.println("MCP3427 Disconnected!");
        Serial.println(" ");
        Serial.println("        ************        ");
        Serial.println(" ");
    }
